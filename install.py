...
def check_shell_config(location):
    # check if the wallacannon alias is in the given file
    with open(location, 'rt') as f:
        check = 'alias wallacannon' in f.read()
    return check
...
def handle_shell():
    ...
    command = 'echo \'alias wallacannon="python3 ~/.wallacannon/wallacannon.py"\'' 
...
def main():
    ...
    Restart the terminal and type {Fore.YELLOW}wallacannon {Fore.GREEN}to run the program{Fore.WHITE}
    """)
    exit()
